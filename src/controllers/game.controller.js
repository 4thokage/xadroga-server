...
const Game = require("../domain/game.model");
...
const join = (io, socket, data, liveGames) => {
  liveGames.hgetall(data.gameId, (err, res) => {
    ...
    if (!game) socket.emit("cannot_join_game", "Cannot join! Room ID is invalid");
    else if (game.ongoing) socket.emit("cannot_join_game", "Cannot join! Game has already started");
    else if (game.createdBy.userId === data.userId) socket.emit("cannot_join_game", "Cannot join! C'mon it's your own game");
    else {
...
const move = (io, socket, data, liveGames) => {
  ...
  liveGames.hgetall(gameId, (err, res) => {
    ...
    if (!chess.game_over()) gameState.lastMove = chess.move(pendingMove);
    ...
    if (gameState.lastMove === null) if (validatePawnPromotion(socket, chess, pendingMove)) return;
